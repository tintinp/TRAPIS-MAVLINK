...
//UW END
    ...
    {
        plane.disarm_motors();
        mavlink_custom_trapis_t packet;
        mavlink_msg_custom_trapis_decode(msg, &packet);

        Location new_home_loc {};
        new_home_loc.lat = (int32_t)(packet.lat * 1.0e7f);
        new_home_loc.lng = (int32_t)(packet.lon * 1.0e7f);
        new_home_loc.alt = (int32_t)(packet.alt * 100.0f);
        plane.ahrs.set_home(new_home_loc);
        plane.home_is_set = HOME_SET_NOT_LOCKED;
        plane.Log_Write_Home_And_Origin();
        gcs().send_home(new_home_loc);
